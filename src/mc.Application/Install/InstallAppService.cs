...
using mc.Configuration.Dto;
using mc.Configuration.Host.Dto;
using mc.EntityFrameworkCore;
using mc.Identity;
using mc.Install.Dto;
using mc.Migrations.Seed;
using mc.Migrations.Seed.Host;
...
namespace mc.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : mcAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<mcDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{mcConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{mcConsts.ConnectionStringName}", constring);
        }
